...
exports.handler = async (event) => {
  console.log('=== COMPLETE POST MULTIPART UPLOAD STARTED ===');
  ...
  try {
    ...
    console.log('Completing post multipart upload:', {
      bucketName,
      fileName,
      uploadId,
    ...
    console.log('Post multipart upload completed successfully');
    ...
    return {
      ...
      body: JSON.stringify({
        message: 'Post multipart upload completed successfully',
    ...
    console.error("Error completing post multipart upload:", err);
    return {
      statusCode: 500,
      headers: {
        'Access-Control-Allow-Origin': '*',
      ...
      body: JSON.stringify({ 
        message: "Error completing post multipart upload", 
        error: err.message 
      })
    };
  }
