...
exports.handler = async (event) => {
  console.log('Events webhook received:', JSON.stringify(event, null, 2));
  ...
  try {
    if (stripeEvent.type === 'checkout.session.completed') {
      ...
      if (session.mode === 'payment' && session.metadata && session.metadata.eventId) {
        ...
        console.log('Processing event creation payment:', { eventId, userId });
        
        // Create the event in PostgreSQL database
        ...
        try {
          ...
          // Get the event draft from the session metadata
          const eventDraft = session.metadata.eventDraft ? JSON.parse(session.metadata.eventDraft) : null;
          
          if (!eventDraft) {
            console.error('No event draft found in session metadata');
            return {
              statusCode: 400,
              headers: corsHeaders,
              body: JSON.stringify({ error: 'No event draft found in session metadata' })
            };
          }
          
          const query = `
            INSERT INTO events (
              id, title, sport, event_type, location, date, time, 
              max_participants, participation_fee, dress_code, 
              description, image_url, coordinates, host_id, 
          ...
          const values = [
            ...
            eventDraft.date,
            eventDraft.time,
            eventDraft.maxParticipants,
            eventDraft.participationFee,
            eventDraft.dressCode,
            eventDraft.description,
            eventDraft.imageUrl,
            eventDraft.coordinates ? JSON.stringify(eventDraft.coordinates) : null,
            userId,
            'active' // Set status to active after payment
          ];
          ...
          console.error('Failed to create event:', dbErr);
          throw dbErr;
        } finally {
          await client.end();
        }
    ...
    console.log('Events webhook processed successfully');
    return {
      statusCode: 200,
      headers: corsHeaders,
    ...
    console.error('Events webhook processing error:', error);
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({ 
