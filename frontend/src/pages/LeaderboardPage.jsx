...
const LeaderboardPage = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full px-2 sm:px-4 overflow-x-hidden">
      {/* Header */}
      ...
      {/* Top 3 Cards */}
      {activeTab === 'athletes' ? (
        <div className="flex flex-col md:flex-row justify-center items-start gap-4 md:gap-8 mb-8">
          {/* #1 Athlete - left side (on mobile: top) */}
          {top3[0] && (
            <div className="relative bg-white shadow-lg rounded-3xl flex flex-col items-center text-center w-full max-w-xs sm:max-w-sm p-4 mx-auto mb-4 md:mb-0 min-h-[480px]" style={{minHeight: '480px'}}>
              {/* Country code (top right, above name) */}
              <div className="absolute top-2 right-4 text-gray-400 text-sm italic font-semibold tracking-widest">{getCountryAcronym(top3[0].country)}</div>
              ...
              <div className="relative mb-1 mt-1 flex justify-center w-full">
                {top3[0].profilePictureUrl ? (
                  <img
                    ...
                    alt={top3[0].name}
                    className="w-14 h-14 rounded-full object-cover border-4 border-yellow-400 shadow mx-auto cursor-pointer"
                    style={{ zIndex: 1 }}
                    onClick={() => handleViewProfile(top3[0])}
                  />
                ) : (
                  <div className="w-14 h-14 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-lg mr-3">
              ...
              {/* Name */}
              <h2
                className="text-2xl font-extrabold mb-2 tracking-tight w-full text-center cursor-pointer hover:underline"
                style={{overflow: 'visible'}}
                onClick={() => handleViewProfile(top3[0])}
              >
                {top3[0].name}
              </h2>
              {/* Info line - stacked vertically */}
              <div className="flex flex-col items-center text-sm text-gray-400 italic font-semibold mb-1 w-full gap-0.5">
              ...
              <div className="flex flex-col gap-2 w-full mt-6 mb-4 items-center text-base">
                ...
                <div className="flex items-center gap-2 justify-center text-lg">
                  ...
                  <span className="italic font-semibold text-gray-700">Challenges</span>
                </div>
                {/* Approvals - Use API data */}
                <div className="flex items-center gap-2 justify-center text-lg">
                  <span className="text-2xl">🧑‍🏫</span>
                  <span className="font-bold">{top3[0].coachApprovals || 0}</span>
                  <span className="italic font-semibold text-gray-700">Approvals</span>
                </div>
              </div>
              {/* Grinder of the Week */}
              <div className="flex flex-col gap-2 items-center mt-0">
                {grinderOfWeekId && top3.some(u => u.id === grinderOfWeekId) && grinderOfWeekId === top3[0].id && (
                  <span className="w-full max-w-xs md:max-w-[490px] block text-center whitespace-nowrap bg-gradient-to-r from-yellow-400 to-yellow-600 text-white italic font-semibold text-lg px-4 py-1.5 rounded-2xl mt-2" style={{fontStyle: 'italic', fontWeight: 500, minWidth: '200px', width: '100%'}}>Grinder of the Week</span>
                )}
              </div>
            </div>
          )}
          {/* #2 and #3 - right side, stacked (on mobile: below 1st, stacked vertically) */}
          <div className="flex flex-col gap-2.5 w-full max-w-xs sm:max-w-sm items-start mx-auto">
            {[top3[1], top3[2]].filter(Boolean).map((user, idx) => (
              <div key={user.id} className="relative bg-white shadow-lg rounded-3xl flex flex-col items-center text-center w-full max-w-xs sm:max-w-sm p-4 mx-auto mb-2 min-h-[110px]" style={{minHeight: '110px'}}>
                {/* Country code (top right, above name, with spacing) */}
                <div className="absolute top-2 right-4 text-gray-400 text-xs italic font-semibold tracking-widest z-10">{getCountryAcronym(user.country)}</div>
                {/* Profile Pic + Medal (top center) */}
                <div className="relative mb-1 mt-1 flex justify-center w-full">
                  {user.profilePictureUrl ? (
                    <img
                      ...
                      alt={user.name}
                      className={`w-14 h-14 rounded-full object-cover border-4 ${idx === 0 ? 'border-gray-400' : 'border-orange-400'} shadow cursor-pointer`}
                      style={{ zIndex: 1 }}
                  ...
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-7 h-7 z-10 flex items-center justify-center">
                    <div className={`w-7 h-7 rounded-full border ${idx === 0 ? 'border-gray-400' : 'border-orange-400'} bg-white flex items-center justify-center`}>
                      <span className={`w-5 h-5 rounded-full ${idx === 0 ? 'bg-gradient-to-br from-gray-300 to-gray-400 text-gray-700' : 'bg-gradient-to-br from-orange-400 to-orange-600 text-orange-900'} flex items-center justify-center text-xs font-bold border border-white`}>{idx + 2}</span>
                    </div>
                  </div>
                </div>
                {/* Name */}
                <h2
                  className="text-lg font-extrabold mb-0 tracking-tight w-full text-center cursor-pointer hover:underline"
                  onClick={() => handleViewProfile(user)}
                >
                  {user.name}
                </h2>
                {/* Info line - stacked vertically */}
                <div className="flex flex-col items-center text-xs text-gray-400 italic font-semibold mb-0.5 w-full gap-0.5">
                ...
                <div className="relative mb-1 mt-1 flex justify-center w-full">
                  {top3[0].profilePictureUrl ? (
                    <img
                      src={top3[0].profilePictureUrl}
                      alt={top3[0].name}
                      className="w-16 h-16 rounded-full object-cover border-4 border-yellow-400 shadow mx-auto cursor-pointer"
                      style={{ zIndex: 1 }}
                      onClick={() => handleViewProfile(top3[0])}
                    />
                  ) : (
                    <div className="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-lg mr-3">
                      {top3[0].name ? top3[0].name.charAt(0) : 'U'}
                    </div>
                  )}
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-7 h-7 z-10 flex items-center justify-center">
              ...
              {[top3[1], top3[2]].filter(Boolean).map((user, idx) => (
                <div key={user.id} className={`relative bg-white shadow-lg rounded-3xl flex flex-row items-stretch h-[230px] w-full p-0`} style={{minWidth: '430px', width: '430px'}}>
                  ...
                  <div className="flex-1 flex flex-col justify-start pl-2 pr-4 py-1">
                    ...
                    {/* Name */}
                    <div
                      className="font-bold text-base truncate cursor-pointer hover:underline"
                      style={{maxWidth: 120}}
                      onClick={() => handleViewProfile(user)}
                    >
                      {user.name}
              ...
              <div className="w-full flex flex-col items-center">
                {rest.map((user, idx) => {
                  ...
                  return (
                    <div
                      key={user.id}
                      className="flex items-center bg-white rounded-2xl shadow-md mb-4 px-4 py-2 relative w-full h-20 max-w-full"
                    >
                      {/* Rank Number (inside card, left of profile pic) */}
                      <div className="flex-shrink-0 flex flex-col items-center justify-center mr-2" style={{ minWidth: 32 }}>
                        <div className="w-7 h-7 rounded-full bg-gray-200 text-gray-500 font-bold text-base flex items-center justify-center shadow-sm border border-gray-100">
                          {idx + 4}
                        </div>
                      </div>
                      {/* Profile Picture */}
                      {user.profilePictureUrl ? (
                        <img
                          src={user.profilePictureUrl}
                          alt={user.name}
                          className="w-10 h-10 rounded-full object-cover border-2 border-white shadow mr-3 cursor-pointer"
                          onClick={() => handleViewProfile(user)}
                        />
                      ) : (
                        <div
                          className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 font-bold text-lg mr-3 cursor-pointer"
                          onClick={() => handleViewProfile(user)}
                        >
                          {user.name ? user.name.charAt(0) : 'U'}
                        </div>
                      ...
                      <div className="flex-1 flex flex-col justify-center min-w-0">
                        ...
                        <div className="flex flex-row flex-wrap gap-2 text-gray-500 text-xs mt-0.5 items-center">
                          <span>Age: {calculateAge(user.dateOfBirth)}</span>
                          <span>• {user.position}</span>
                          <span>• Level {getLevelFromXP(user.xpTotal || 0)}</span>
                          <span>• {getCountryAcronym(user.country)}</span>
                        </div>
                        <div className="flex flex-row gap-2 text-gray-500 text-xs mt-0.5 items-center">
                          <span>{user.height ? formatHeight(user.height) : '--'}</span>
                          <span>{user.weight ? formatWeight(user.weight) : '--'}</span>
                        </div>
                      </div>
                      {/* Challenges Section */}
                      <div className="flex flex-col items-end justify-center min-w-[90px] ml-2">
                        <span className="font-bold text-base text-gray-700">{user.challengesSubmitted || 0} <span className="font-normal">Challenges</span></span>
                        <div className="w-20 h-2 bg-gray-200 rounded-full mt-1 relative">
                          <div
                            className="absolute left-0 top-0 h-2 rounded-full bg-red-500"
                            style={{ width: `${Math.min((user.challengesSubmitted || 0) / Math.max(...rest.map(u => u.challengesSubmitted || 0), 1) * 100, 100)}%` }}
                          ></div>
                        </div>
                      </div>
                    </div>
