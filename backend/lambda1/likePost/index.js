...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (existingLike.rows.length > 0) {
      ...
      // Send push to post owner for like
      try {
        ...
        if (pushToken) {
          ...
          const message = {
            default: 'Your post was liked',
            APNS: JSON.stringify({ aps: { alert: { title: 'New Like', body: 'Someone liked your post' }, sound: 'default', badge: 1 } }),
            GCM: JSON.stringify({ notification: { title: 'New Like', body: 'Someone liked your post' } }),
          };
          await sns.publish({ Message: JSON.stringify(message), MessageStructure: 'json', TargetArn: pushToken }).promise();
        }
      } catch (e) {
        console.warn('Failed to send like push', e.message);
      }
