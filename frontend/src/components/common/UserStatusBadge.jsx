...
const UserStatusBadge = ({ user, className = '', isOwnProfile = false }) => {
  if (!user) return null;
  ...
  // Only show blue check for verified scouts (regardless of premium)
  if (is_verified && role === 'scout') {
    badgeIcon = (
  ...
  } else if (is_premium_member && (role === 'athlete' || role === 'coach') && isOwnProfile) {
    // Red star for premium athletes/coaches - only show on own profile
    badgeIcon = (
